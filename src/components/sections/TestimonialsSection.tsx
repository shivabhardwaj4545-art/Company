        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <span className="text-xs font-black tracking-widest text-[#A3E635] uppercase flex items-center gap-2 mb-1">
              <MessageSquareQuote className="w-4 h-4 text-[#A3E635]" /> SOCIAL PROOF &amp; RECEIPTS
            </span>
          ...
          <p className="text-sm sm:text-base text-[var(--text-secondary)] max-w-md">
            Unfiltered feedback from founders, product leads, and executives we have collaborated with. Drag horizontally to explore.
          </p>
        </div>
      </div>

      {/* Horizontally Draggable Card Scroller */}
      <div className="flex gap-6 overflow-x-auto no-scrollbar px-4 sm:px-8 lg:px-12 pb-6 pt-2 cursor-grab active:cursor-grabbing select-none">
        {testimonials.map((t) => (
          <div
            key={t.id}
            data-cursor="hover"
            className="w-[340px] sm:w-[380px] shrink-0 p-6 rounded-2xl border border-[var(--border)] bg-[var(--surface)] accent-glow-hover flex flex-col justify-between"
          >
            <div>
